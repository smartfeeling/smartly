    ...
    private byte[] merge(final Resource resource,
        ...
        try {
            ...
            if (session.isNew()) {
                ...
                // session.setAttribute("velocity-engine", VLCManager.getInstance().getEngine().getNativeEngine());
            }

            final Map<String, Object> sessionContext = (Map<String, Object>) session.getAttribute("velocity-context");
            final VelocityEngine engine = getEngine(); //(VelocityEngine) session.getAttribute("velocity-engine");
